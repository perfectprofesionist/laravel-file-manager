<!-- Main view for displaying shared files and folders -->
@extends('layouts.app')
                                        ...
                                        <ol class="breadcrumb custom-breadcrumb">
                                            ...
                                            @foreach ($parents as $parent)
                                                <li class="breadcrumb-item">
                                                    ...
                                                    @else
                                                        <a
                                                            href="{{ route('list.contents.shared', $parent['guid']) }}">{{ $parent['name'] }}</a>
                                                        <!-- Parent folder link -->
                                                    @endif
                                                </li>
                                            @endforeach
                                                ...
                                                <thead>
                                                    <tr>
                                                        <th class="sort" data-sort="name"
                                                            ...
                                                            <a href="{{ route('list.contents.shared', [$sharedFolderGuid, 'order' => request('order') === 'asc' ? 'desc' : 'asc', 'sort' => 'name']) }}">
                                                                Name
                                                        ...
                                                        <th class="sort" data-sort="updated_at"
                                                            data-order="{{ request('order') === 'asc' ? 'desc' : 'asc' }}">
                                                            <a href="{{ route('list.contents.shared', [$sharedFolderGuid, 'order' => request('order') === 'asc' ? 'desc' : 'asc', 'sort' => 'updated_at']) }}">
                                                                Date Modified
                                                ...
                                                <tbody class="content-list">
                                                    ...
                                                    @if ($sharedContents->count() > 0)
                                                        @foreach ($sharedContents as $accessControl)
                                                            <tr>
                                                                <td>
                                                                    @if ($accessControl->content->is_folder ?? $accessControl->is_folder)
                                                                        <!-- Folder link -->
                                                                        <a href="{{ route('list.contents.shared', $accessControl->content->guid ?? $accessControl->guid) }}"
                                                                            class="custmclckbtnshared">
                                                                            <b><img src="{{ $accessControl->content->svgPath ?? $accessControl->svgPath ? asset($accessControl->content->svgPath ?? $accessControl->svgPath) : asset('assets/images/folder.svg') }}"
                                                                                    alt="FolderSvg"
                                                                                    style="width: 20px; height: 20px;">
                                                                                {{ $accessControl->content->name ?? $accessControl->name }}
                                                                            </b>
                                                                        </a>
                                                                    @else
                                                                        <!-- File display -->
                                                                        <b><img src="{{ $accessControl->svgPath ?? $accessControl->content->svgPath ? asset($accessControl->svgPath ?? $accessControl->content->svgPath) : asset('assets/images/folder.svg') }}"
                                                                                alt="{{ $accessControl->extension ?? $accessControl->content->extension }}"
                                                                                style="width: 20px; height: 20px;">
                                                                            {{ $accessControl->content->name ?? $accessControl->name }}
                                                                        </b>
                                                                    @endif
